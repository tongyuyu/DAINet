...
class FAM(nn.Module):
    def __init__(
        ...
        self.convto32 = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=False)

    def forward(self,  x, depth,):
        #x = self.DoubleAttention(x)
        depth = self.DoubleAttention(depth)
        out = torch.mul(x,depth)
        out = self.convto32(out)
...
class Network(nn.Module):
    def __init__(self, channel=32,imagenet_pretrained=True,mode=True):
        ...
        #upsample function
        self.upsample1_1 = nn.Sequential( nn.ConvTranspose2d(96, 32, kernel_size=2, stride=2, padding=0, bias=False), nn.BatchNorm2d(32),nn.PReLU())
    ...
    def forward(self, x, x_depth):
        ...
        x4 = self.conv2048to32(x4)
        x3 = self.conv1024to32(x3)
        x2 = self.conv512to32(x2)
        x4_depth = self.conv2048to32(x4_depth )
        x3_depth = self.conv1024to32(x3_depth )
        x2_depth  = self.conv512to32(x2_depth )
        ...
        #
        x2_3 = torch.cat((x2,self.upsample2(x3),self.upsample4(x4)),1) 


        y = self.upsample1_1(self.upsample4(x2_3)) #
